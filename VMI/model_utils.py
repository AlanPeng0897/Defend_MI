...
def _load_cls(dataset, cls_path, device):
    ...
    #             classifier = ResNetCls1(3, zdim=model_args['latent_dim']).to(device)
        ...
        #     C,H,W = 3,64,64
        #     classifier = ResNetCls1(3, zdim=model_args['latent_dim'], imagesize=64, nclass=1000,
        #                             resnetl=model_args['resnetl'], dropout=model_args['dropout']).to(device)
        # else:
        #     C, H, W = 1, 32, 32
...
def load_cls_embed(dataset, cls_path, device, classify=False, logits=False):
    ...
    if dataset == 'mnist':
        def extract_feat(x):
            ...
            return func((x.view(x.size(0), C, H, W)))
...
def instantiate_generator(args, device):
    if args.gen in ['basic', 'conditional', 'conditional_no_embed']:
        generator = nets.ConditionalGenerator(args.imageSize, args.nz, args.ngf, args.nc, args.n_conditions,
                                              args.gen in ['conditional', 'conditional_no_embed'], args.g_sn,
                                              args.g_z_scale, args.g_conditioning_method,
                                              args.gen != 'conditional_no_embed',
                                              norm=args.g_norm, cdim=args.cdim).to(device)
    elif args.gen in ['secret', 'secret-conditional']:
        generator = nets.ConditionalGeneratorSecret(args.imageSize, args.nz, args.ngf, args.nc, args.n_conditions,
                                                    args.gen == 'secret-conditional', args.g_sn, args.g_z_scale,
                                                    args.g_conditioning_method, args.gen != 'conditional_no_embed',
                                                    norm=args.g_norm, cdim=args.cdim).to(device)
    elif args.gen == 'toy':
        generator = nets.ConditionalGeneratorToy(args.imageSize, args.nz, args.ngf, args.nc, args.n_conditions,
                                                 args.use_labels, args.g_sn, args.g_z_scale,
                                                 args.g_conditioning_method).to(device)
    else:
        raise ValueError()
    return generator
...
def instantiate_discriminator(args, index2class, device):
    disc_config = yaml.safe_load(open(f'disc_config/{args.disc_config}', 'r'))
...
class TargetDatasetReconstructionEvaluation:
    def __init__(self, evaluation_classifier, target_data_x, target_data_y, bgr=True, run_target_feat_eval=True, k=3,
                 factor=1, manifold_type='knn'):
        ...
        if run_target_feat_eval:
            with torch.no_grad():
                bs = 500
                self.target_data_z = torch.cat([
                    self.evaluation_classifier.embed(self.target_data_x[start:start + bs, [2, 1, 0]
