...
def run_kde(x_train, x_eval, device, logvar=None, B=100, l2=False, return_details=False, detach=False, is_print=True,
            agg='logmeanexp', topk=5):
...
def main(args):
    ...
    if args.model in ['l2_aux', 'dcgan_aux', 'kplus1gan']:
        ...
        nclass = len(torch.unique(
            experiment.target_dataset['Y_train'])) if 'nclass' not in experiment.target_dataset else \
            experiment.target_dataset['nclass']
    ...
    vutils.save_image(dat['X_train'][:100], '%s/data-train.jpeg' % args.output_dir, normalize=True, nrow=10)
    vutils.save_image(dat['X_train'][-100:], '%s/data-train1.jpeg' % args.output_dir, normalize=True, nrow=10)
    if dat['X_test'] is not None:
        vutils.save_image(dat['X_test'][:100], '%s/data-test.jpeg' % args.output_dir, normalize=True, nrow=10)
        vutils.save_image(dat['X_test'][:50], '%s/data-test5.jpeg' % args.output_dir, normalize=True, nrow=5)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch in [20, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume_from_local_ckpt', type=int, default=1)
    ...
    # print('SLURM_JOB_ID' in os.environ)
    # exit()
