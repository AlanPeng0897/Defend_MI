...
if __name__ == '__main__':
    ...
    if args.dataset == 'celeba':
        ...
        g_path = "./result/models_cxr_gan/cxr_G_100.tar"
        G = generator.GeneratorCXR()
        G = nn.DataParallel(G).cuda()
        ckp_G = torch.load(g_path)
        G.load_state_dict(ckp_G['state_dict'], strict=False)

        d_path = "./result/models_cxr_gan/cxr_D_100.tar"
        D = discri.DGWGAN(1, 64)
        D = nn.DataParallel(D).cuda()
        ckp_D = torch.load(g_path)
        D.load_state_dict(ckp_D['state_dict'], strict=False)
        if args.defense == 'HSIC' or args.defense == 'COCO':
            ...
            for (a1, a2, ac) in hp_ac_list:
                print("a1:", a1, "a2:", a2, "test_acc:", ac)
                T = model.VGG16_CXR(num_classes, True)
                T = nn.DataParallel(T).cuda()
                ...
                for idx in range(5):
                    print("--------------------- Attack batch [%s]------------------------------" % idx)
                    acc, acc5, var, var5 = inversion(args, G, D, T, E, iden,
                                                     iter_times=1500, num_seeds=100, verbose=False)

                    fid_value = calculate_fid_given_paths0([f'attack_res/{args.dataset}/trainset/',
                                                            f'attack_res/{args.dataset}/{args.defense}/all/'],
                                                           50, 1, 2048)
                    print(f'FID:{fid_value:.4f}')
            ...
            if args.defense == "vib":
                beta = args.beta = 0.03
                ac = args.acc = 39.43
                print(f'beta:{beta}, acc:{ac}')
                # path_T = os.path.join(args.model_path, args.dataset, args.defense, "VGG16_beta0.020_40.44.tar")
                path_T = os.path.join(args.model_path, args.dataset, args.defense,
                                      f"{model_name}_beta{beta:.3f}_{ac:.2f}.tar")
                T = model.VGG16_CXR_vib(num_classes)
                T = nn.DataParallel(T).cuda()
                ...
                T = model.VGG16_CXR(num_classes=7)
